...
% load data
load('./det_face_task_diff.mat');
nSubj = size(det_face_task_diff, 3);
...
mean_HR_det_diff = squeeze(mean(det_face_task_diff(1,:,:), 3));
sderr_HR_det_diff = squeeze(std(det_face_task_diff(1,:,:), [], 3) / sqrt(nSubj));
...
% do fit
[xData, yData] = prepareCurveData(x, mean_HR_det_diff);
...
mseb(xData', yData', sderr_HR_det_diff, lineProps);
...
%% prepare permutation
shuffled_HR_det_diff = label_permutation_func(det_face_task_diff, n_perm);

mean_HR_det_shuffled_diff = squeeze(mean(shuffled_HR_det_diff(1,:,:,:), 3));

%% do perm

for iPerm = 1:n_perm
    
    [xData, yData] = prepareCurveData(x, mean_HR_det_shuffled_diff(:, iPerm));
