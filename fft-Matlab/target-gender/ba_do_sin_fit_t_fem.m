...
nFem = size(det_face_task_t_fem, 3);
...
mean_HR_det_t_fem = squeeze(mean(det_face_task_t_fem(1,:,:), 3));
sderr_HR_det_t_fem = squeeze(std(det_face_task_t_fem(1,:,:), [], 3) / sqrt(nFem));
...
% do fit
[xData, yData] = prepareCurveData(x, mean_HR_det_t_fem);
...
mseb(xData', yData', sderr_HR_det_t_fem, lineProps);
...
%% prepare permutation
shuffled_HR_det_t_fem = label_permutation_func(det_face_task_t_fem, n_perm);

mean_HR_det_shuffled_t_fem = squeeze(mean(shuffled_HR_det_t_fem(1,:,:,:), 3));

%% do perm

for iPerm = 1:n_perm
    
    [xData, yData] = prepareCurveData(x, mean_HR_det_shuffled_t_fem(:, iPerm));
