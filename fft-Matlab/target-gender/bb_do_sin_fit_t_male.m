...
nMale = size(det_face_task_t_male, 3);
...
mean_HR_det_t_male = squeeze(mean(det_face_task_t_male(1,:,:), 3));
sderr_HR_det_t_male = squeeze(std(det_face_task_t_male(1,:,:), [], 3) / sqrt(nMale));
...
% do fit
[xData, yData] = prepareCurveData(x, mean_HR_det_t_male);
...
mseb(xData', yData', sderr_HR_det_t_male, lineProps);
...
%% prepare permutation
shuffled_HR_det_t_male = label_permutation_func(det_face_task_t_male, n_perm);

mean_HR_det_shuffled_t_male = squeeze(mean(shuffled_HR_det_t_male(1,:,:,:), 3));

%% do perm

for iPerm = 1:n_perm
    
    [xData, yData] = prepareCurveData(x, mean_HR_det_shuffled_t_male(:, iPerm));
