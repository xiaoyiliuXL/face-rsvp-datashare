...
load('HR_diff_mat.mat');
% 1 row: age
% 2 row: face task acc
% 3 row: gender task acc
% 4 row: refresh rate

nSubj = size(HR_diff_mat, 3);

face_task_diff = squeeze(HR_diff_mat([2 4], :, :));

%% detrend at individual level
det_face_task_diff = nan(size(face_task_diff)); 
trend_face_task_diff = nan(size(face_task_diff, [2 3]));
% summary_face = []; summary_face.adjR2 = nan(nSubj, 1); summary_face.coeffs = nan(nSubj, 4);

for iSubj = 1: nSubj
    
    curr_face_dat = squeeze(face_task_diff(:,:,iSubj));
    ...
    det_face_task_diff(1,:,iSubj) = curr_det_face'; 
    det_face_task_diff(2,:,iSubj) = repelem(unique(curr_face_dat(2,:)), 20); 
    trend_face_task_diff(:,iSubj) = fit_curve;
...
save('./det_face_task_diff.mat', 'det_face_task_diff')
...
for iSubj = 1:nSubj
    ...
    curr_face_dat = squeeze(det_face_task_diff(:,:,iSubj)); 
...
%     end
    ...
    if iSubj == 1
        spctr_face_diff.cmplx_out = nan(size(curr_spctr_face.cmplx_out, 1), nSubj);
        spctr_face_diff.spctr_out = nan(size(curr_spctr_face.spctr_out, 1), nSubj);
    end
    
    spctr_face_diff.cmplx_out(:, iSubj) = curr_spctr_face.cmplx_out;
    spctr_face_diff.spctr_out(:, iSubj) = curr_spctr_face.spctr_out;
    
end
spctr_face_diff.freqs = curr_spctr_face.freqs;
mask_freq_interest = (unique(spctr_face_diff.freqs) >= min(foi)) & (unique(spctr_face_diff.freqs) <= max(foi));

%% see what's going on
% ampiltude
amp_face_diff = mean(spctr_face_diff.spctr_out, 2);

% phase locked amplitude
PL_face_diff = abs(sum(spctr_face_diff.cmplx_out, 2))/nSubj;

%% Permutation
[amp_mat_perm_diff, PL_mat_perm_diff] = deal(nan([size(spctr_face_diff.spctr_out),...
    n_perm]));

HR_mat_perm_diff = label_permutation_func(det_face_task_diff, n_perm);
...
for iPerm = 1:n_perm
    ...
    for iSubj = 1:nSubj
        ...
        curr_face_perm = HR_mat_perm_diff(1,:,iSubj,iPerm);
        curr_refresh_rate_perm = HR_mat_perm_diff(2,1,iSubj,iPerm);
...
%         end
        ...
        % big mat
        amp_mat_perm_diff(:,iSubj,iPerm) = curr_spctr.spctr_out;
        PL_mat_perm_diff(:,iSubj,iPerm) = curr_spctr.cmplx_out;
...
%% 95% permutation
% get group average
perm_amp_face_diff = squeeze(mean(amp_mat_perm_diff(:,:,:), 2));
perm_PL_face_diff = squeeze(abs(sum(PL_mat_perm_diff(:,:,:), 2)) / nSubj);

% get 95%
amp_face_95_diff = prctile(perm_amp_face_diff, 95, 2);
PL_face_95_diff = prctile(perm_PL_face_diff, 95, 2);
...
plot(unique(spctr_face_diff.freqs), PL_face_diff, 'Color', colors(1, :), 'LineWidth', 7)
plot(unique(spctr_face_diff.freqs), PL_face_95_diff, 'r--', 'LineWidth', 4)
...
delete(ax)
[peakfreqs_face, cntr_freq_face, pval_face, Zcirc_face] = peakfreqs(spctr_face_diff, pol_ax, ...
...
%% p value
pvals_face_diff = compute_pvals_and_mc(PL_face_diff, ...
    perm_PL_face_diff, ...
    spctr_face_diff.freqs)
