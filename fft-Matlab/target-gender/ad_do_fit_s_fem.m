...
load('HR_s_fem_mat.mat');
% 1 row: age
% 2 row: face task acc
% 3 row: gender task acc
% 4 row: refresh rate

nFem = size(HR_s_fem_mat, 3);

face_task_s_fem = squeeze(HR_s_fem_mat([2 4], :, :));

%% detrend at individual level
det_face_task_s_fem = nan(size(face_task_s_fem)); 
trend_face_task_s_fem = nan(size(face_task_s_fem, [2 3]));
% summary_face = []; summary_face.adjR2 = nan(nSubj, 1); summary_face.coeffs = nan(nSubj, 4);

for iSubj = 1: nFem
    
    curr_face_dat = squeeze(face_task_s_fem(:,:,iSubj));
    ...
    det_face_task_s_fem(1,:,iSubj) = curr_det_face'; 
    det_face_task_s_fem(2,:,iSubj) = repelem(unique(curr_face_dat(2,:)), 20); 
    trend_face_task_s_fem(:,iSubj) = fit_curve;
...
%% Do fft
spctr_face_s_fem = []; spctr_gender_t_fem = [];

for iSubj = 1:nFem
    
    curr_face_dat = squeeze(det_face_task_s_fem(:,:,iSubj)); 
...
%     end
    ...
    if iSubj == 1
        spctr_face_s_fem.cmplx_out = nan(size(curr_spctr_face.cmplx_out, 1), nFem);
        spctr_face_s_fem.spctr_out = nan(size(curr_spctr_face.spctr_out, 1), nFem);
    end
    
    spctr_face_s_fem.cmplx_out(:, iSubj) = curr_spctr_face.cmplx_out;
    spctr_face_s_fem.spctr_out(:, iSubj) = curr_spctr_face.spctr_out;
    
end
spctr_face_s_fem.freqs = curr_spctr_face.freqs;
mask_freq_interest = (unique(spctr_face_s_fem.freqs) >= min(foi)) & (unique(spctr_face_s_fem.freqs) <= max(foi));

%% see what's going on
% ampiltude
amp_face_s_fem = mean(spctr_face_s_fem.spctr_out, 2);

% phase locked amplitude
PL_face_s_fem = abs(sum(spctr_face_s_fem.cmplx_out, 2))/nFem;

%% Permutation
[amp_mat_perm_s_fem, PL_mat_perm_s_fem] = deal(nan([size(spctr_face_s_fem.spctr_out),...
    n_perm]));

HR_mat_perm_s_fem = label_permutation_func(det_face_task_s_fem, n_perm);
...
for iPerm = 1:n_perm
    ...
    for iSubj = 1:nFem
        ...
        curr_face_perm = HR_mat_perm_s_fem(1,:,iSubj,iPerm);
        curr_refresh_rate_perm = HR_mat_perm_s_fem(2,1,iSubj,iPerm);
...
%         end
        ...
        % big mat
        amp_mat_perm_s_fem(:,iSubj,iPerm) = curr_spctr.spctr_out;
        PL_mat_perm_s_fem(:,iSubj,iPerm) = curr_spctr.cmplx_out;
...
%% 95% permutation
% get group average
perm_amp_face_s_fem = squeeze(mean(amp_mat_perm_s_fem(:,:,:), 2));
perm_PL_face_s_fem = squeeze(abs(sum(PL_mat_perm_s_fem(:,:,:), 2)) / nFem);

% get 95%
amp_face_95_s_fem = prctile(perm_amp_face_s_fem, 95, 2);
PL_face_95_s_fem = prctile(perm_PL_face_s_fem, 95, 2);
...
plot(unique(spctr_face_s_fem.freqs), PL_face_s_fem, 'Color', colors(1, :), 'LineWidth', 7)
plot(unique(spctr_face_s_fem.freqs), PL_face_95_s_fem, 'r--', 'LineWidth', 4)
...
delete(ax)
[peakfreqs_face, cntr_freq_face, pval_face, Zcirc_face] = peakfreqs(spctr_face_s_fem, pol_ax, ...
...
%% p value
pvals_face_s_fem = compute_pvals_and_mc(PL_face_s_fem, ...
    perm_PL_face_s_fem, ...
    spctr_face_s_fem.freqs)
