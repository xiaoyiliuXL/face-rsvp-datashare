...
mean_HR_det = squeeze(mean(det_face_task(1,:,:), 3));
sderr_HR_det = squeeze(std(det_face_task(1,:,:), [], 3) / sqrt(nSubj));
...
% do fit
[xData, yData] = prepareCurveData(x, mean_HR_det);
...
mseb(xData', yData', sderr_HR_det, lineProps);
...
%% prepare permutation
shuffled_HR_det = label_permutation_func(det_face_task, n_perm);

mean_HR_det_shuffled = squeeze(mean(shuffled_HR_det(1,:,:,:), 3));

%% do perm

for iPerm = 1:n_perm
    
    [xData, yData] = prepareCurveData(x, mean_HR_det_shuffled(:, iPerm));
