...
%% DETRENDED FIGURE
load('det_face_task_sum.mat');
load('det_gender_task_sum.mat');

det_face_avg_mat = mean(det_face_task(1,:,:),3)';
det_face_sderr_mat = (squeeze(std(det_face_task(1,:,:),[],3))/sqrt(nsubjs))';

det_gender_avg_mat = mean(det_gender_task(1,:,:),3)';
det_gender_sderr_mat = (squeeze(std(det_gender_task(1,:,:),[],3))/sqrt(nsubjs))';
...
lineProps_face.style = '.';
mseb(xvals', det_face_avg_mat, det_face_sderr_mat', lineProps_face, 0.7);

hold on;
plot(xvals, det_face_avg_mat, 'Color', colors(1,:), 'LineWidth',7)

hold on;
lineProps_gender.col = {colors(2,:)};
lineProps_gender.transparent = .01;
lineProps_gender.style = '.';
mseb(xvals', det_gender_avg_mat, det_gender_sderr_mat', lineProps_gender, 0.7);

hold on;
plot(xvals, det_gender_avg_mat, 'Color', colors(2,:), 'LineWidth',7)
