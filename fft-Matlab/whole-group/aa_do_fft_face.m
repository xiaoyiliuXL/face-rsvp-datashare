...
face_task = squeeze(big_HR_mat([2 4], :, :));

%% detrend at individual level
det_face_task = nan(size(face_task)); 
trend_face_task = nan(size(face_task, [2 3]));
% summary_face = []; summary_face.adjR2 = nan(nSubj, 1); summary_face.coeffs = nan(nSubj, 4);

for iSubj = 1: nSubj
    
    curr_face_dat = squeeze(face_task(:,:,iSubj));
    ...
    det_face_task(1,:,iSubj) = curr_det_face'; 
    det_face_task(2,:,iSubj) = repelem(unique(curr_face_dat(2,:)), 20); 
    trend_face_task(:,iSubj) = fit_curve;
...
plot(curr_time_bin, mean(squeeze(det_face_task(1,:,:)), 2)); hold on
plot(curr_time_bin, mean(trend_face_task, 2)); hold on
plot(curr_time_bin, mean(squeeze(face_task(1,:,:)), 2))

save('./det_face.mat', 'det_face_task')
%load('./det_face.mat');
...
%% Do fft
spctr_face = []; spctr_gender = [];

for iSubj = 1:nSubj
    
    curr_face_dat = squeeze(det_face_task(:,:,iSubj)); 
...
%     end
    ...
    if iSubj == 1
        spctr_face.cmplx_out = nan(size(curr_spctr_face.cmplx_out, 1), nSubj);
        spctr_face.spctr_out = nan(size(curr_spctr_face.spctr_out, 1), nSubj);
        spctr_face.cmplx_out_long = nan(size(curr_spctr_face.cmplx_out_long, 1), nSubj);
    end
    
    spctr_face.cmplx_out(:, iSubj) = curr_spctr_face.cmplx_out;
    spctr_face.spctr_out(:, iSubj) = curr_spctr_face.spctr_out;
    spctr_face.cmplx_out_long(:, iSubj) = curr_spctr_face.cmplx_out_long;
    
end
spctr_face.freqs = curr_spctr_face.freqs;
mask_freq_interest = (unique(spctr_face.freqs) >= min(foi)) & (unique(spctr_face.freqs) <= max(foi));

%% see what's going on
% ampiltude
amp_face = mean(spctr_face.spctr_out, 2);

% phase locked amplitude
PL_face = abs(sum(spctr_face.cmplx_out, 2))/nSubj;

%% Permutation
[amp_mat_perm, PL_mat_perm] = deal(nan([size(spctr_face.spctr_out),...
    n_perm]));

HR_mat_perm = label_permutation_func(det_face_task, n_perm);
...
for iPerm = 1:n_perm
    ...
    for iSubj = 1:nSubj
        ...
        curr_face_perm = HR_mat_perm(1,:,iSubj,iPerm);
...
%% 95% permutation
% get group average
perm_amp_face = squeeze(mean(amp_mat_perm(:,:,:), 2));
perm_PL_face = squeeze(abs(sum(PL_mat_perm(:,:,:), 2)) / nSubj);

% get 95%
amp_face_95 = prctile(perm_amp_face, 95, 2);
PL_face_95 = prctile(perm_PL_face, 95, 2);
...
plot(unique(spctr_face.freqs), PL_face, 'Color', colors(1, :), 'LineWidth', 7)
plot(unique(spctr_face.freqs), PL_face_95, 'r--', 'LineWidth', 4)
...
%% p value
pvals_face = compute_pvals_and_mc(PL_face, ...
    perm_PL_face, ...
    spctr_face.freqs)
